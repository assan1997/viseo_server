...
module.exports = {
  ...
  async addNewContact(req, res) {
    ...
    if (data.pseudo !== data.user_pseudo) {
      ...
      if (contact !== null) {
        let result = userContact.find((c) => c.login === data.pseudo);
        let index = userContact.findIndex((c) => c.login === data.pseudo);
        if (result !== null) {
          userContact.splice(userContact(indexOf(index), contact));
          saveContact(params);
        } else {
          saveContact(userContact, (contact = contact));
        }
      }
      async function saveContact(params, contact = null) {
        await User.updateOne(
          { login: data.user_pseudo },
          { contacts: [...params, `${contact !== null ? '' : contact}`] }
        );
