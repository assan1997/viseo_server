...
io.on('connection', function (socket) {
  ...
  socket.on('ok', function (data) {
    ...
    let peer = clients.find((c) => c.username === data.peer);
    OnCallStatus(true, (init = init), (peer = peer));
  ...
  socket.on('end', function (data) {
    let init = clients.find((c) => c.username === data.init);
    let peer = clients.find((c) => c.username === data.peer);
    OnCallStatus(false, (init = init), (peer = peer));
    io.to(init.room).emit('initEnd');
    io.to(peer.room).emit('peerEnd');
  });

  socket.on('reload', function (data) {
    OnCallStatus(false, (init = data.user));
  });
});

function OnCallStatus(status, init = null, peer = null) {
  if (init !== null) {
