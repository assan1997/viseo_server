...
io.on('connection', function (socket) {
  ...
  socket.on('sendMessage', function (data) {
    ...
    let body = { sendBy: data.header.emitter }
    let message = { ...data, sendBy: };

    io.to(init.room).emit('updateMessages', message);
    if (peer !== undefined) io.to(peer.room).emit('updateMessages', message);
  });
});
