...
io.on('connection', function (socket) {
  ...
  socket.on('call', function (data) {
    ...
    if (peer === undefined) {
      ...
      if (peer.oncall) {
        feedback.msg = 'user has another call ...';
        feedback.status = 'failed';
        socket.emit('call-event', feedback);
      }
      socket.emit('call-event', feedback);
  ...
  socket.on('ok', function (data) {
    ...
    let iniIndex = clients.findIndex((i) => i.username === data.init);
    let peer = clients.find((c) => c.username === data.peer);
    let peerIndex = client.find((c) => c.username === data.peer);

    init.oncall = true;
    clients.splice(indexOf(iniIndex), init);
    peer.oncall = true;
    clients.splice(indexOf(peerIndex), peer);
...
function callStatus(status, init, peer) {
  let iniIndex = clients.findIndex((i) => i.username === init.username);
  let peerIndex = client.find((c) => c.username === peer.username);
  init.oncall = false;
